...
dates <-
  tibble(
    ...
    ban_year = c(2016, 2014, 2014, 2016, 2014), 
    passage_year = c(2014, 2011, 2013, 2014, 2012)
  )
...
fixest::etable(lm_state,lm_counties, lm_state_ca, lm_state_ct, lm_state_ma,lm_state_ri,lm_state_vt,vcov="cluster",  tex= TRUE, digits = "r3", digits.stats = "r2")
...
pa_trend_all_c <- feols(tons_pc~ seq+cat |state_id, dt_did %>% filter(year<2014)) 
...
pa_trend_all <- feols(tons_pc~ seq+cat |state_id, dt_did_state %>% filter(year<2014)) 
...
etable(pa_trend_all, pa_trend_all_c, pa_trend_ca,pa_trend_ct, pa_trend_ma, pa_trend_ri, pa_trend_vt, vcov = "cluster", tex= TRUE, digits = "r3", digits.stats = "r2") #parallel trends assumption really holds only for MA



# in the text, we write: 
#Fourth, our results replicate with a simple two-way fixed effects difference-in-differences analysis (Table \ref{tab:did}). For Connecticut, although we cannot reject parallel trends in the pre-treatment period, we also cannot rule out a spurious sizable treatment effect stemming from non-parallel trends. Specifically, to check for parallel trends before the bans, we regress the log of disposal on a common linear trend, $\delta$, (for the treatment and the control group), and a treatment-group linear trend, $\omega$. We find that $\delta=-0.0344^{***}$ (with st. error 0.0034) and $\omega=-0.0012$ (with st. error $0.0034$). A back-of-the-envelope calculation reveals that this difference in trends over five years (2014--2018) could produce spurious treatment effects similar to the estimates ($(-0.0012-0.0034\cdot2)\cdot 5=-4\%$).

delta = pa_trend_ct$coefficients[1] %>% round(4)

omega = pa_trend_ct$coefficients[2] %>% round(4)
omega_std = pa_trend_ct$sigma2 %>% round(4)
