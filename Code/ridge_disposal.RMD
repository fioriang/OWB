...
## Functions
```{r}
# Understanding the ridge Placebo Approach
ridge_pre <- function (k, dt,ban_year,offset, donors)
{
  ...
  a1 <-ridge_att(y_train, y_test,y_att, x_train, x_test, x_att, test_ind_end1, test_ind_end2)
  #a2 <-ridge_att_modification(y_train, y_test,y_att, x_train, x_test, x_att, test_ind_end1, test_ind_end2)
  
  a <- 
    a1 %>% matrix(nrow=1, ncol=5) %>%    
    as_data_frame() %>%  
    rename(
      att = V1, 
      mape = V2, 
...
ridge_att <- function(y_train, y_test,y_att, x_train, x_test, x_att, test_ind_end1, test_ind_end2)
{
...
ridge_att_modification <- function(y_train, y_test,y_att, x_train, x_test, x_att, test_ind_end1, test_ind_end2)
{
  cv_model <- cv.glmnet(x_train, y_train, alpha = 0, dfmax = 5)
  best_lambda <- cv_model$lambda.min
  best_model <- glmnet(x_train, y_train, alpha = 0, lambda = best_lambda, dfmax = 5)
  x_with_coef_test <- cbind(1,x_test)
  x_with_coef_att <- cbind(1,x_att)
...
get_plac_res_alt_state_county_ridge <- function (dt,r_threshold, mape_threshold )
{
...
pool_function_ridge <-function(i, data, donors, r_threshold,mape_threshold)
{
  donors <- tibble(
    county_id =donors, 
...
## Basic
```{r}
ridge_att_res <- lapply(1:length(donors), ridge_pre,dt,ban_year=2015,offset=4, donors)
ridge_att_res_2015 <- ridge_att_res %>%  bind_rows()


ridge_att_res <- lapply(1:length(donors), ridge_pre,dt,ban_year=2014,offset=4, donors)
ridge_att_res_2014 <- ridge_att_res %>%  bind_rows()


ridge_att_res <- lapply(1:length(donors), ridge_pre,dt,ban_year=2016,offset=4, donors)
ridge_att_res_2016 <- ridge_att_res %>%  bind_rows()

ridge_att_res <- rbind(ridge_att_res_2015, ridge_att_res_2014, ridge_att_res_2016)

write.csv(ridge_att_res, "ridge_att_res.csv", row.names = FALSE)
...
ridge_state_county <- get_plac_res_alt_state_county_ridge(ridge_att_res, r_threshold = 0, mape_threshold = Inf)
...
```{r}
pool_estimates <- lapply(seq(1:length(donors)), pool_function_ridge, data = ridge_att_res, donors, r_threshold = 0, mape_threshold = Inf)
pool_estimates <- pool_estimates %>% bind_rows()


write.csv(pool_estimates, "ridge_pool_estimates.csv", row.names = FALSE)
```
...
## Basic
```{r}
ridge_att_res <- lapply(1:length(donors_state), ridge_pre,dt_state,ban_year=2015,offset=4, donors_state)
ridge_att_res_2015 <- ridge_att_res %>%  bind_rows()


ridge_att_res <- lapply(1:length(donors_state), ridge_pre,dt_state,ban_year=2014,offset=4, donors_state)
ridge_att_res_2014 <- ridge_att_res %>%  bind_rows()


ridge_att_res <- lapply(1:length(donors_state), ridge_pre,dt_state,ban_year=2016,offset=4, donors_state)
ridge_att_res_2016 <- ridge_att_res %>%  bind_rows()

ridge_att_res_state <- rbind(ridge_att_res_2015, ridge_att_res_2014, ridge_att_res_2016)

write.csv(ridge_att_res_state, "ridge_att_res_state.csv", row.names = FALSE)
...
```{r}
pool_estimates_state <- lapply(seq(1:length(donors_state)), pool_function_ridge, data = ridge_att_res_state, donors_state, r_threshold = 0, mape_threshold = Inf)
pool_estimates_state <- pool_estimates_state %>% bind_rows()

write.csv(pool_estimates_state, "ridge_pool_estimates_state.csv", row.names = FALSE)
...
## County ## 
ridge_att_res <- read.csv("ridge_att_res.csv")
...
spec2 <- ridge_state_county %>%
  mutate(specification= "County Pooled State")

## County Pooled##
ridge_pool_estimates <- read.csv("ridge_pool_estimates.csv")

spec3 <- ridge_pool_estimates %>%
  mutate(specification= "County Pooled")

## State ##
ridge_att_res_state <- read.csv("ridge_att_res_state.csv")

spec4 <- ridge_att_res_state %>%
  mutate(specification= "State")

## State Pooled ## 
ridge_pool_estimates_state <- read.csv("ridge_pool_estimates_state.csv")

spec5 <- ridge_pool_estimates_state %>%
...
ci_specifications_ridge <-
  ...
  mutate(
    ...
    ci_level = rep(c("lower", "median","upper"),5),
...
ci_specifications_lasso_ridge <- ggpubr::ggarrange(ci_specifications_ridge, ci_specifications_LASSO,  heights = c(1,2), ncol = 1)

ggsave(ci_specifications_lasso_ridge, filename = "ci_specifications_lasso_ridge.pdf", device = cairo_pdf,
       path= figure_path,
