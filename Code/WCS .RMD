...
wcs_plot <- 
  ...
  labs (x="", y="Fraction of total landfill MSW (%)", fill = "Commercial:")+
  theme(
...
## Table 1: Expected Regulations Effect
...
disposal_effect_size %>% 
  ...
  mutate(
    ...
    Covered = ifelse(state_id=="CT", 100*(comp_paper_share+food_share), 
                     ifelse(state_id=="CA", Organic, Food))
  ) %>% select(state_id, year, MSW, `Com. MSW`, Organic, Food, Coverage, Disposal, Composting, Covered) %>%
  group_by(state_id) %>%
  mutate(phase = cumsum(Coverage != lag(Coverage, default = first(Coverage))) + 1) %>%
  ungroup() %>% 
  group_by(state_id, MSW, `Com. MSW`, Organic, Food, Covered, phase) %>% 
  summarise(
    year = min(year),
  ...
  select(state_id, phase,  year, Coverage, null_column2, Covered, null_column1, Disposal) %>%
  {
...
municipal <- 
  ...
  left_join(
    wcs %>%group_by(state_id) %>% filter(year ==max(year)) %>% select(-year) %>%  filter(state_id %in% c("M1", "M2")), #basically assume that percentages hold
    by = c("state_id")
  ) %>% 
