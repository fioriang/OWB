...
main_path <- "/Users/fian4421/Library/CloudStorage/Dropbox/OWB2/data/01.state_level_data"
figure_path <- "/Users/fian4421/Library/CloudStorage/Dropbox/Apps/Overleaf/OWB_vol2/Figures"
main_owb2_path <- "/Users/fian4421/Library/CloudStorage/Dropbox/OWB2/data/01.state_level_data"
```
...
msw_frac <- texas %>% filter(year>2010) %>% summarise(msw_percentage=mean(msw_percentage)) %>% pluck("msw_percentage")
...
colorado <- 
  ...
  mutate(tons=tons*msw_frac) %>% #usual MSW fraction
  select(year, state_id, county_name,type, tons) %>%
  rbind (
    colorado_recycling %>% 
      mutate(
...
connecticut <- 
  ...
  rbind ( 
    composting %>% 
      ...
      left_join(., counties_cities, by =c("town")) %>% 
      group_by(year, county) %>% 
      summarise( tons = sum(tons, na.rm = TRUE)) %>% 
...
illinois <- 
  ...
  summarise(tons = sum(tons)) %>% 
  mutate(tons = tons*(0.754+0.818)/2) # avg msw percentage between 2 state WCS
...
il_imports <- 
  ...
  ungroup %>% 
  select(year, county_name, tons, destination, origin, source_data)%>% 
  mutate(tons = tons*(0.754+0.818)/2) # avg msw percentage
...
michigan <- 
  ...
  mutate(
    tons = ifelse(is.na(tons_2),tons, tons*0.75 + tons_2)
  ) %>%  
  select(-tons_2)
...
# Nevada -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# These in-advertedely use total waste data not only MSW data
...
nevada_comp <- 
  ...
  group_by(year,state_id, county_name, type) %>%
  summarise(tons = sum(tons, na.rm = TRUE)) %>% 
  filter(type %in% c("recycling", "composting"))


path <- paste0(main_owb2_path, "/nv/waste/")
data <- read.csv(paste0(path,"treated.csv")) %>%  as_tibble()
...
ny <-
  read.csv(paste0(state_data_path,"/New York/import_R/ny.csv")) %>% 
  rename(
...
# New York ##############
path <- paste0(state_data_path, "/New York/import_R/")
ny <- read.csv(paste0(path,"ny.csv"))

## In the OWB code, I do not filter based on waste type. this might have implications for NY imports used to estimate exported waste from CT. This is why I save here the ny_imports_old and the ny_imports_diff so that I re-calculate CT's waste disposal 
ny <-
  ny %>% 
  rename(
    waste_type = Waste.Type, 
...
ny <-
  ...
  select(
    year, waste_type, tons, county_name, state_id, Facility.Name
  ) %>%  as_tibble %>% 
  filter(waste_type == "MSW (Residential/Institutional & Commercial)") %>% 
  mutate(
    state_id = ifelse(state_id == "QC", "CAN", state_id),
    state_id = ifelse(state_id %in% c("Not Reported", "not reported") & county_name=="Canada", "CAN", state_id), 
    county_name = str_to_lower(county_name)
...
ny <- 
  ny %>% 
  # group_by(year, state_id, county_name, Facility.Name, tons) %>% 
  # summarise(n=n()) %>% 
  # filter(n==1) %>% select(-n) %>% 
  # rbind(
  #   ny %>% 
...
path <- paste0(main_owb2_path, "/ny/waste/")
data <- read.csv(paste0(path, "treated.csv")) %>% as_tibble()
imports <- read.csv(paste0(path, "imports.csv")) %>% as_tibble()
exports <- read.csv(paste0(path, "exports.csv")) %>% as_tibble()


# we noticed that data for Hempstead Resource Recovery Facility are missing in 2021-2022. So we use the mean for 2020, 2023 to supplement these data: 


missing_data <- 
  data %>% 
  filter(year==2021| year==2020 | year==2022 | year==2023) %>%
  group_by(year, facility_name, activity_number) %>% 
  summarise(tons = sum(tons)) %>% 
  pivot_wider(names_from = year, values_from = tons) %>% 
  ungroup %>% 
  mutate(
    factor_2022 = `2022`/ `2023`, 
    factor_2021 = `2021`/ `2020`
  ) %>% 
  filter(
    is.na(`2021`) | is.na(`2020`) | is.na(`2022`)| is.na(`2023`) | factor_2022 < 0.2 | factor_2021 < 0.2) %>%
  mutate(
    is_missing = ifelse((is.na(`2021`) | is.na(`2022`)) & !is.na(`2023`) & !is.na(`2020`), 1, 0), # a record is deemed missing if its NA in 2021 or 2022 and is not NA in 2023 and in 2020
    factor_2021 =  ifelse(is.na(factor_2021), 1, factor_2021), 
    factor_2022 =  ifelse(is.na(factor_2022), 1, factor_2022), 
    is_missing = ifelse(factor_2021< 0.2 | factor_2022 < 0.2, 1, is_missing)
  ) %>% 
  filter(is_missing==1) %>% 
  rowwise() %>% 
  mutate(
    `2021` = ifelse(factor_2021 < 0.2, NA, `2021`), # if something is reported but not fully (for example for Wheelabrator Whestchester LP in 2020 and in 2023 we have >540,000 tons and in 2021 we only have 30K), 
    `2022` = ifelse(factor_2022 < 0.2, NA, `2022`), # if something is reported but not fully (for example for Wheelabrator Whestchester LP in 2020 and in 2023 we have >540,000 tons and in 2021 we only have 30K), 
    supplemented_2021 = ifelse(is.na(`2021`), "yes", "no"),
    supplemented_2022 = ifelse(is.na(`2022`), "yes", "no"),
    `2021` = ifelse(`2021` %>% is.na(), mean(c(`2020`,`2023`)), `2021`), 
    `2022` = ifelse(`2022` %>% is.na(), mean(c(`2020`,`2023`)), `2022`)
  ) %>% 
  pivot_longer(
  ...
  filter(
    (year==2022 & supplemented_2022=="yes")| year!=2022
  ) %>% 
  filter(
    (year==2021 & supplemented_2021=="yes")| year!=2021
  ) %>% 
  select(facility_name, activity_number, year, tons)




# Take the pattern of 2020 and for the activities with missing data in 2021 assign the tons in the relevant counties 
missing_data_2021 <- 
  data %>% 
  filter(year==2020) %>% 
  filter(
    activity_number %in% c(missing_data %>% filter(year==2021) %>% pluck("activity_number"))
  ) %>% 
  group_by(
    facility_name, activity_number, county
  ) %>% 
  summarise(
    tons = sum(tons)
  ) %>% 
  group_by(
    facility_name, activity_number
  ) %>% 
  mutate(
    total_activity_tons = sum(tons), 
    factor = tons/total_activity_tons
  ) %>% 
  left_join(
    missing_data %>% filter(year==2021) %>% select(facility_name, activity_number, tons, year) %>% 
      rename(tons_missing = tons), 
    by = c("facility_name", "activity_number")
  ) %>% 
  mutate(
    tons = tons_missing * factor
  )
...
missing_data_2022 <- 
  ...
  filter(year==2023) %>% 
  filter(
    activity_number %in% c(missing_data %>% filter(year==2022) %>% pluck("activity_number"))
  ) %>% 
  group_by(
    facility_name, activity_number, county
  ) %>% 
  ...
  left_join(
    missing_data %>% filter(year==2022) %>% select(facility_name, activity_number, tons, year) %>% 
      rename(tons_missing = tons), 
    by = c("facility_name", "activity_number")
  ) %>% 
  mutate(
...
data <- 
  rbind(
    #from the original dataset remove year 2021's and 2022's activities that we deemed missing, because we will add them later
    data %>% 
      filter(year==2021) %>% 
      filter(!activity_number %in% (missing_data %>% filter(year==2021) %>% pluck("activity_number") %>% unique())), 
    
    
    data %>% 
      filter(year==2022) %>% 
      filter(!activity_number %in% (missing_data %>% filter(year==2022) %>% pluck("activity_number"))), 
...
ny_post_owb <- 
  ...
  # group_by(year, state_id, county_name, facility_name, tons) %>%
  # summarise(n=n()) %>% select(-n) %>% 
  group_by(county_name, state_id, year) %>% 
  summarise(
    tons = sum(tons)
  ) %>% 
  mutate(
  ...
  rbind(
    rbind(
      missing_data_2021, missing_data_2022
    ) %>% 
      ungroup %>% 
      select(year, county, tons) %>% 
      mutate(
        state_id="NY",
        county_name = str_to_lower(county), 
        type = "disposal", 
        year= year %>% as.double()
      ) %>% 
      select(
...
ny <- 
 ...
 ny <-
  ny %>%
  mutate(
    county_name = ifelse(county_name == "orange ", "orange", county_name)
  ) %>%
  filter(county_name!= "not reported") %>%
  left_join(
    ny_exports %>% group_by(year) %>% summarise(exported_tons=sum(tons)),
    by = c("year")
  ) %>%
  left_join(
    population %>%
      group_by(year, state_id) %>% mutate(state_pop = sum(pop)),
    by = c("year", "state_id", "county_name")
  ) %>%
  mutate(
    tons = tons + exported_tons * pop/state_pop
  ) %>%
  select(year, state_id, county_name, type, tons)
...
ny_imports <- 
  ...
  group_by(year, county_name, destination, origin, source_data) %>% 
  summarise(tons = sum(tons))%>% 
  select(year, county_name, tons, origin, destination, source_data) 

# ny_imports_ct_diff <- 
#   ny_imports_old %>% filter(origin == "CT") %>%  select(year, tons) %>% 
#   left_join(
#     ny_imports %>% filter(origin == "CT") %>% ungroup %>%  select(year, tons) %>% rename(new_tons = tons)
#   ) %>% 
#   mutate(
#     diff= new_tons- tons
#   )
...
# North Carolina -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# OWB data 
path <- paste0(state_data_path, "/North Carolina/import_R/")
nc <- read.csv(paste0(path, "nc.csv"))
nc$county_name <- tolower(nc$county_name)
nc$year <- nc$year +1
nc <- as.data.frame(nc %>% group_by (year, county_name)%>% summarize(weight=sum(weight)))

nc <- nc[,c("year", "county_name", "weight")]
...
#three sets of data: 
#treated which includes information from 2019-2023
#treated_2012_2018 which includes information from 2012-2018
#treated_2011_2006 which includes information from 2006-2011

path <- paste0(main_path, "/nc/")
data1 <- read.csv(paste0(path, "waste/treated.csv")) %>% as_tibble() %>% 
  mutate(year=year+1)
data2 <- read.csv(paste0(path, "waste/treated_2012_2018.csv")) %>% as_tibble() %>% 
  mutate(year=year+1)
data3 <- read.csv(paste0(path, "waste/treated_2011_2006.csv")) %>% as_tibble() %>% 
  mutate(
    total = ifelse(year==2005, NA, total)
  ) %>% 
  mutate(year=year+1) %>% select(county_name, year, type, total)
...
data_nc3 <- 
  ...
  filter(type=="cd") %>% 
  mutate(
    county_name = county_name %>% str_to_lower(), 
    state_id="NC", 
    type = "cd", 
    waste = ifelse(is.na(waste) & year!=2006, 0, waste), 
  ) %>% 
  select(
    state_id, county_name, year, type, waste 
  )  %>% 
  group_by(state_id, county_name, year, type) %>% 
  summarise(waste = sum(waste)) %>% 
  group_by(state_id, county_name, type) %>% 
  mutate(
    waste = ifelse(year==2006, 0.5*lag(waste, order_by = year)+ 0.5*lead(waste, order_by = year), waste), # because FY 2005 (i.e., year ==2006 here, year==2005 in raw csv) are not available, take the mean between the next and the following
  ) %>% 
  ungroup %>% 
  rbind(
    data2 %>% 
      filter(
        year <= 2016, 
        type=="cd"
      ) %>% #before 2015 i only have the cd not the msw, cd
      rename(waste=tons) %>% 
      mutate(
        county_name = county_name %>% str_to_lower(), 
        state_id="NC", 
        waste = ifelse(is.na(waste), 0, waste), 
        export = ifelse(is.na(export), 0, export), 
        import =ifelse(is.na(import), 0, import), 
        waste = waste + export - import
      ) %>%
      group_by(state_id, county_name, type) %>% 
      ungroup %>% 
      select(
...
#For the data before 2016, subtract the cd from the previous waste
data_nc <- 
  nc %>% 
  filter(year<= 2016) %>% 
  left_join(
    data_nc3 %>% select(-type), by = c("year", "state_id", "county_name")
  ) %>% 
  filter(year>=2006) %>% 
  mutate(
    waste=ifelse(is.na(waste), 0, waste), 
  ...
  group_by(state_id, county_name, type) %>% 
  mutate(
    waste = zoo::rollmean(waste, k = 2, fill = waste) # this is needed because NC reports waste by financial year, not calendar year
  )


nc <- data_nc %>% rename(tons = waste)

################## imports and exports
path <- paste0(state_data_path, "/North Carolina/import_R/")
nc_imports <- read.csv(paste0(path,"nc_imports_exports.csv")) %>% as_tibble()
...
path <- paste0(main_path, "/nc/")
data <- read.csv(paste0(path, "imp_exp/imp_exp.csv")) %>% as_tibble() 
...
imp_nc <- 
  ...
  group_by(state_id, year, origin, destination, imp_exp, waste_type) %>% 
  summarise(
    tons=sum(tons)
  ) %>% 
  ungroup %>% 
  filter(waste_type=="msw") %>% 
  mutate(
    county_name = NA, 
    source_data  = "NC", 
    origin = "unknown") %>% #because before 2020 i have only unknown origin, I will do that for the post 2020 data as well
  group_by(year, county_name, destination, origin, source_data) %>% 
  summarise(
    tons = sum(tons)
  ) %>% ungroup %>%   
...
#get the msw- non msw fraction of imports 
msw_frac_imp_exp_nc <- 
  ...
  group_by(state_id, year, imp_exp, waste_type) %>% 
  summarise(
    tons=sum(tons)
  ) %>% group_by(year, imp_exp) %>% 
  mutate(msw_frac = tons/sum(tons)) %>% 
  filter(waste_type=="msw") %>% group_by(imp_exp) %>% 
  summarise(msw_frac=mean(msw_frac))
...
exp_nc <- 
  ...
  ungroup %>% 
  filter(waste_type=="msw") %>% 
  mutate(county_name = NA, source_data  = "NC") %>% 
  select(year, county_name, tons, destination, origin, source_data)



## combine the two (owb and post-owb)

nc_imports <- 
  rbind(nc_imports, imp_nc) %>% 
  mutate(
    year=year+1
  ) %>% 
  group_by(origin, destination) %>% 
  mutate(
    tons = ifelse(year< 2020, tons * (msw_frac_imp_exp_nc %>% filter(imp_exp=="import") %>% pluck("msw_frac")), tons)
  ) %>% 
  mutate(
...
nc_exports <- 
  rbind(nc_exports, exp_nc) %>% 
  mutate(
    year=year+1
  ) %>% 
  group_by(origin, destination) %>% 
  mutate(
    tons = ifelse(year< 2020, tons * (msw_frac_imp_exp_nc %>% filter(imp_exp=="export") %>% pluck("msw_frac")), tons)
  ) %>%
  mutate(
    tons = zoo::rollmean(tons, k = 2, fill = tons)
  ) %>% ungroup
...
oklahoma <-
  ...
  group_by (year, state_id, county_name, type) %>% 
  summarise (tons = sum(tons, na.rm=TRUE)) %>% 
  mutate(tons=msw_frac*tons) #usual percentage of MSW
...
pen <- 
  ...
  filter(!county_name %in% c("canada","disrict of columbia", "puerto rico"))
...
#make it into calendar year, when it says 2017 it means july-dec 2016 and jan-jun 2017
sc <- 
  sc %>%  
  group_by(county_name, type) %>% 
  mutate(
    tons = zoo::rollmean(tons, k = 2, fill = tons) # this way we create waste for 2017 by taking 1/2 of 2017 and 1/2 of 2018
  )
...
tn <- 
  tn %>% 
  rbind(
    # knox county does not report data in 2015, so we add the mean from 2014 and 2016
    rbind(
      tn %>% filter(county_name == "knox", year==2014, type=="disposal"), 
      tn %>% filter(county_name == "knox", year==2016, type=="disposal")
    ) %>% 
      mutate(year=2015) %>% 
      group_by(year, state_id, county_name, type) %>% 
      summarise(
    ...
    # madison county does not report data in 2006--2008, use the 2009 data 
    rbind(
      tn %>% filter(county_name == "madison", year==2009, type=="disposal"), 
    ) %>% ungroup %>% select(-year) %>% 
      expand_grid(year=c(2006, 2007, 2008)) %>% 
      select(year, state_id, county_name, type)
  )
...
utah <- 
  ...
  rename (
    ...
    county_name = county, 
    year = reportPeriod) %>% 
  filter(facilityType %in% c("Incinerator", "Compost", "Landfill", "Recycler")) %>%
  pivot_longer(
    cols = c("disposal", "recycling", "composting"), 
    names_to = "type", 
    values_to = "tons") %>%
  filter(!is.na(tons)) %>% 
  mutate(county_name = str_remove(county_name, "County") %>% str_trim()) 
...
utah <-
  ...
  summarise(tons = sum(tons, na.rm=TRUE)) %>%
  mutate(tons = ifelse(near(tons, 0), NA, tons)) %>%
  group_by(county_name) %>%
  pivot_wider(names_from = year, values_from = tons) %>%
  pivot_longer(-county_name, names_to = "year", values_to = "tons") %>%
  mutate(tons = na.approx(tons, x = year, na.rm = FALSE)) %>% # if for a county we have NA, replace with the linear approximation between adjacent values
  fill(tons, .direction = "down") %>% # if its at the end of the time series, replace NA with the last known number
  mutate(
    type = "disposal",
    year = year %>% as.integer()
  ) %>%
  # all the above do only for disposal, for non-dispsoal leave as is
  rbind(
    utah %>%
      group_by(year, county_name, type, tons) %>%
      summarize(number=n()) %>%
...
utah %>% filter(type=="disposal", year>2005) %>% group_by(year) %>% summarise(tons = sum(tons))
...
#because for WI export data we do not have post-2017 data, i assume the same ratios and supplement it. 
wi_exports_assumptions <- 
  wi_exports %>% filter(year==max(year)) %>% 
  rename(
    export_tons = tons
  ) %>%
  select(year, destination, export_tons) %>% 
  left_join(
    wi %>% filter(type=="disposal") %>% ungroup %>% select(year, tons), 
    by = "year"
  ) %>% 
  mutate(
    frac = export_tons/tons
  ) %>% #until here, we get the fraction of 2017 disposal that is epxorted to IA, IL, MI
  select(destination, frac) %>% 
  cross_join( # here we want to apply this fraction to later years
    wi %>% filter(type=="disposal", year>2017) %>% ungroup %>% select(year, tons), 
  ) %>% 
  mutate(
    tons = tons*frac, 
    county_name = NA, 
...
ct_exports <-
  ...
  mutate(
   exports = ifelse(destination%in% c("MA") & !is.na(exports_2) & year>2014, exports_2, exports), 
   exports = ifelse(destination%in% c("NY") & !is.na(exports_2) & year>2014, exports_2, exports), 
   exports = ifelse(destination%in% c("OH") & !is.na(exports_2) & year>2014, exports_2, exports),
   ) %>%
  mutate(
    county_name = NA,
    source_data = "CT",
...
power2 <- power2 %>% filter(state_id!="CT") 
power2 <- rbind(power2, connecticut)


power2 %>% filter(state_id=="CT", year==2018, type == "disposal") %>% summarise(sum(tons))
power2 %>% filter(state_id=="CT", year==2006, type == "disposal") %>% summarise(sum(tons))
#2679243
#2380291
```
...
states_need_export_data <- c("CO", "GA", "IL", "IN", "KY", "MO", "MS", "PA", "RI", "TX", "AZ", "OK", "AL", "MI") 

states_need_import_data <- c("CO", "GA", "MO", "RI", "TN", "IL", "AZ","ME", "OK") # illinois needs after 2012
...
```{r}
power2_imp_exp <- power2_imp_exp %>% filter((state_id=="DE" & year>=2009)| state_id!="DE") # for the years prior to 2009 we do not have the annual reports

#power2_imp_exp <- power2_imp_exp %>% filter((state_id=="CT" & year<2019)| state_id!="CT") # for CT save only until 2019 
...
## Validation of imp exp

```{r}
data_states <- power2 %>% filter(year>2010, type=="disposal") %>% summarise(state_id =unique(state_id)) %>% pluck("state_id")

imports_check <- imports %>% filter(source_data!="CT") %>% rbind(ct_imports)
exports_check <- exports %>% filter(source_data!="CT") %>% rbind(ct_exports)


# To check imports i have to go to imports and do destination 

state_id_given <- "NY"

check_fun_exporting <- function (state_id_given)
{
  imports %>% 
    filter(origin %in% data_states, destination %in% data_states) %>% 
    group_by(year, destination, origin, source_data) %>% 
    summarise(tons = sum(tons, na.rm = TRUE)) %>% 
    filter(origin == state_id_given, year > 2009) %>% 
    group_by(year, origin, destination) %>% 
    summarise(from_imports = sum(tons)) %>% 
    left_join( 
      exports %>% 
        filter(origin %in% data_states, destination %in% data_states) %>% 
        #filter(destination=="NY"& year>2011|destination!="NY") %>% 
        #filter(destination=="NY" & origin=="VT"| destination!="NY") %>% 
        group_by(year, destination, origin, source_data) %>% 
        summarise(tons = sum(tons, na.rm = TRUE)) %>% 
        filter(origin == state_id_given, year > 2009) %>% 
        group_by(year, origin, destination, source_data) %>% 
        summarise(from_exports = sum(tons)), 
      by = c("year", "origin", "destination")
    ) %>% 
    mutate(state_id_given) %>% 
    filter(!is.na(from_imports), from_imports>0, 
           !is.na(from_exports), from_exports>0)
}

check_fun_importing <- function (state_id_given)
{
  imports %>% 
    filter(origin %in% data_states, destination %in% data_states) %>% 
    group_by(year, destination, origin, source_data) %>% 
    summarise(tons = sum(tons, na.rm = TRUE)) %>% 
    filter(destination == state_id_given, year > 2009) %>% 
    group_by(year, origin, destination) %>% 
    summarise(from_imports = sum(tons)) %>% 
    left_join( 
      exports %>% 
...
check_fun_ct <- function (state_id_given)
{
  # imports_check %>% 
  #   filter(origin %in% data_states, destination %in% data_states) %>% 
  #   group_by(year, destination, origin, source_data) %>% 
  #   summarise(tons = sum(tons, na.rm = TRUE)) %>% 
  #   filter(origin == state_id_given|destination==state_id_given, year > 2005) %>% 
  #   group_by(year, origin, destination) %>% 
  #   summarise(from_imports = sum(tons)) %>% 
  #   left_join( 
  #     exports_check %>% 
  ...
  imports_check %>% 
    ...
    group_by(year, destination, origin, source_data) %>% 
    summarise(tons = sum(tons, na.rm = TRUE)) %>% 
    filter(origin == state_id_given, year > 2010) %>% 
    group_by(year, origin, destination) %>% 
    summarise(from_imports = sum(tons)) %>% 
    left_join( 
      exports_check %>% 
...
check_fun_ct_imp <- function (state_id_given)
{
  ...
  imports_check %>% 
    ...
    group_by(year, destination, origin, source_data) %>% 
    summarise(tons = sum(tons, na.rm = TRUE)) %>% 
    filter(origin == state_id_given, year > 2005) %>% 
    group_by(year, origin, destination) %>% 
    summarise(from_imports = sum(tons)) %>% 
    full_join( 
      exports_check %>% 
...
check_fun_ct_exp <- function (state_id_given)
{
  ...
  imports_check %>% 
    ...
    group_by(year, destination, origin, source_data) %>% 
    summarise(tons = sum(tons, na.rm = TRUE)) %>% 
    filter(destination == state_id_given, year > 2005) %>% 
    group_by(year, origin, destination) %>% 
    summarise(from_imports = sum(tons)) %>% 
    full_join( 
      exports_check %>% 
...
validate_imports_1 <- lapply(data_states, check_fun_exporting) %>% bind_rows() %>% as_tibble()
validate_imports_2 <- lapply(data_states, check_fun_importing) %>% bind_rows() %>% as_tibble()
...
validate_imports_1 %>% 
  ...
  left_join(
    power2 %>% 
      filter(year>2009, type%in%c("disposal", "msw_disposed")) %>% 
      group_by(state_id, year) %>% 
      summarise(tons= sum(tons, na.rm = TRUE)), 
    by = c("state_id_given"="state_id", "year")) %>%
  filter(year<=2018) %>% 
  group_by(year,state_id_given, tons) %>% 
  summarise(
    from_imports=sum(from_imports),
    from_exports=sum(from_exports)) %>% 
  mutate(
    val=(from_imports-from_exports)/ from_exports,
    val2=(from_imports-from_exports)/ tons,
    ) %>% 
  group_by(state_id_given, year) %>% 
  summarise(
    m=median(val,na.rm=TRUE), 
    m2=median(val2,na.rm=TRUE), 
    m3 = median (abs(val), na.rm=TRUE), 
    m4 = median (abs(val2), na.rm=TRUE)) %>%
  ungroup %>% 
  filter(state_id_given!="CT") %>% 
  #group_by(year)
  summarise(mean(m), mean(m2), mean (m3), mean(m4))
...
validate_imports_2 %>% 
  ...
  filter(year<=2018) %>% 
  group_by(year,state_id_given, tons) %>% 
  summarise(from_imports=sum(from_imports),from_exports=sum(from_exports)) %>% 
  mutate(
    val=(from_imports-from_exports)/ from_imports,
    val2=(from_imports-from_exports)/ tons,
    ) %>% 
  group_by(state_id_given, year) %>% 
  summarise(
...
state_id_given = "PA"
check_fun_ct_imp(state_id_given) %>% 
  filter(year< 2019, year>2009) %>%  filter(!source_data %in% c("CT")) %>% 
  left_join(
    power2 %>% 
      filter(year>=2005, type%in%c("disposal", "msw_disposed")) %>% 
      group_by(state_id, year) %>% 
      summarise(tons= sum(tons, na.rm = TRUE)), 
    by = c("state_id_given"="state_id", "year")) %>%
  group_by(year,state_id_given, tons) %>% 
...
state_id_given = "PA"
check_fun_ct_exp(state_id_given) %>% 
  filter(year< 2019, year>2009) %>%  filter(!source_data %in% c("CT")) %>% 
  left_join(
    power2 %>% 
      filter(year>=2005, type%in%c("disposal", "msw_disposed")) %>% 
...
imports %>%filter(year>2010) %>%  filter(source_data=="OH", origin== "MA") %>% arrange(year)

exports %>%filter(year>2010) %>%  filter(source_data=="MA", destination == "OH") %>% arrange(year)
