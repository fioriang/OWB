...
for (seed in 6:10)
{
  ...
  power_state_fun2(power_state_plac3 %>% bind_rows() %>% mutate(treated_state=="CT"), "CT")
  ...
  write.csv(
    ...
    paste0("power_state_plac_2025_seed", seed, ".csv"), row.names=FALSE
  )
...
power_state1_alt <- power_state_fun2_alternative(power_state_plac_data %>% filter(treated_state=="MA"), "MA") %>% ungroup %>% filter(att_min == max(att_min)) # the optimal number of donors changes from 9 to 4 (ATT = -9.8)
power_state2_alt <- power_state_fun2_alternative(power_state_plac_data %>% filter(treated_state=="CA"), "CA")%>% ungroup %>% filter(att_min == max(att_min))  # the optimal number of donors remains the same (10)
power_state3_alt <- power_state_fun2_alternative(power_state_plac_data %>% filter(treated_state=="CT"), "CT")%>% ungroup %>% filter(att_min == max(att_min)) # the optimal number of donors changes from 3 to 8 (ATT is: -2.73) (calc with seed = 2) (based on seed it kind of changes largest is -5.1 smallest is -2)
power_state4_alt <- power_state_fun2_alternative(power_state_plac_data %>% filter(treated_state=="RI"), "RI")%>% ungroup %>% filter(att_min == max(att_min)) # the optimal number of donors changes from 10 to 6 (ATT is: -3.28)
power_state5_alt <- power_state_fun2_alternative(power_state_plac_data %>% filter(treated_state=="VT"), "VT")%>% ungroup %>% filter(att_min == max(att_min)) # the optimal number of donors remains the same 8 

# all the results continue to hold
```
