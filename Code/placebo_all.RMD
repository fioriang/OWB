...
treat_state <- function (i, treated_state, dt_state_initial)
{
  ...
  tr <- lapply(k,in_sample_R2_tr,dt_state, donors = donors_state, iterations=5000, option="V2", offset=3, samp = c(3:10))
...
treat_county <- function (i, treated_state, dt_initial)
{
  ...
  tr <- lapply(which(treated_counties_id %in% k),in_sample_R2_tr,dt, donors, iterations=3000, option="V2", offset=3, samp = c(3:10))
...
pre_processing_dt_state <- function (power2)
{
  ...
  year_cutoff <- 2018
  dt_state <- 
    power2 %>% mutate(county_id=paste0(county_name, state_id)) %>% 
...
```{r}
plac_for_histogram1 <- read.csv("plac_for_histogram1.csv")
plac_for_histogram2 <- read.csv("plac_for_histogram2.csv")
plac_for_histogram3 <- read.csv("plac_for_histogram3.csv")
plac_for_histogram4 <- read.csv("plac_for_histogram4.csv")
plac_for_histogram5 <- read.csv("plac_for_histogram5.csv")
...
chosen_sample_size =
  tibble(
    read.csv("power_state.csv") %>% filter(specification=="State") %>% group_by(treated_state) %>% filter(att_min == max(att_min)) %>% select(sample_size, treated_state) %>% 
      mutate(sample_size= ifelse(treated_state=="MA", 7, sample_size)) %>% mutate(sample_size= ifelse(treated_state=="CT", 5, sample_size)) %>% 
      rename(
        state_id=treated_state, 
        chosen_sample_size=sample_size),
    actual_mape = c(0.57, 1.17, 2.58, 6.59, 1.51)
  )
...
mape_state %>% 
  ...
  group_by(treated_state) %>%
  summarise(
    mae_low = quantile(mape, 0.025) %>% round(4), 
    mae_high = quantile(mape, 0.975)%>% round(4)) 
...
bt_with_power<- 
  ...
  theme(
    ...
    panel.grid.major.y = element_blank(),
    panel.background = element_blank(), 
    text = element_text(family = "Helvetica",size = 22), 
