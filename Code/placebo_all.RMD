...
in_sample_R2_v2 <- function (k, dt, donors,iterations_scale, option, ban_year, offset, samp, seed)
{
  ...
  {
    sample_size <- samp[f]
    iterations <- 100000 #min(iterations_scale* sample_size, 15000)
...
in_sample_R2_v2_include_treated <- function (k, dt, donors,iterations_scale, option, ban_year, offset, samp)
{
  ###
  # This function creates the SC using the identification method we describe in the paper 
  # It returns for each sample_size the 100 best SC (best means that these SCs have the lowest MAPE)
  ###
  treated_counties_id <- all_treated[which(all_treated != treated_state)]
  treated_location <- donors[k] #the placebo state
  year_end <- ban_year-offset # the start of the validation period
  state_treated <- ifelse( #the placebo state, this serves the case where the treated location is a county and thus we want to exclude all the counties in the same state
    str_length(treated_location)>2, 
...
power_state_plac_include_treated <- function(treated_state, dt_state_initial, seed)
{
  ###
  ...
  plac <- lapply(seq(1:length(donors_state)),in_sample_R2_v2_include_treated,dt_state, donors=donors_state, iterations_scale=100000, option="V2", ban_year=ban_year, offset=3, samp =samp)
...
power_state_plac <- function(treated_state, dt_state_initial, seed)
{
  ...
  }else if(treated_state == "VT")
  {
    dt_state <- dt_state_initial %>%group_by(state_id) %>% 
      mutate(
        tons_pc = tons_pc*0.5 + 0.5*ifelse(is.na(lead(tons_pc, n=1, default = NA)), tons_pc, lead(tons_pc, n=1, default = NA)) 
      )
  }else if(treated_state == "CA")
  {
    dt_state <- dt_state_initial %>%group_by(state_id) %>% 
  ...
  plac <- lapply(seq(9),in_sample_R2_v2,dt_state, donors=donors_state, iterations_scale=100000, option="V2", ban_year=ban_year, offset=3, samp =samp, seed)
  
  
  plac %>% bind_rows() %>% as_tibble() %>% filter(sample_size!=0) %>% group_by(sample_size) %>% filter(mape==min(mape)) %>% summarise(att=mean(att))
...
power_state_fun2_alternative <- function(plac, treated_state)
{
  ...
  spec4 <- 
    ...
    group_by(sample_size, ban_year) %>%     
    summarise(
      att_min = (sort(att)[2] + sort(att)[1])/2, 
      att_max = (sort(att, decreasing=TRUE)[2] + sort(att, decreasing=TRUE)[1])/2, 
      att_median = mean(att)
    ) 
...
power_county_for_hist <- function(treated_state, dt_initial, seed)
{
  ...
  plac <- lapply(seq(1:length(donors)),in_sample_R2_v2,dt, donors=donors, iterations_scale=10000, option="V2", ban_year=ban_year, offset=3, samp =samp, seed = seed)
...
```{r}
#For disposal, we drop counties with very low minimal disposal ($<0.2$)  or very low average disposal ($<0.3$).The first filter excludes 12 counties and the second excludes 9 counties.
...
counties_with_low_disposal <- dt %>% filter(tons_pc <= 0.2)
dt <- dt %>% filter(tons_pc > 0.2)
...
counties_with_late_year_start <- exc %>%  filter(m>=0.3)
counties_with_low_avg_disposal <- exc %>% filter(m<0.3) 
...
counties_with_incomplete_panels <- exc
...
counties_that_exclude_threshold <- exclude2
instances_that_catch_threshold <- (f %>% filter(percentage_1 > threshold | percentage_1 < -threshold)%>% nrow())/(f %>% filter(!(percentage_1 > threshold | percentage_1 < -threshold)) %>% nrow() + f %>% filter(percentage_1 > threshold | percentage_1 < -threshold)%>% nrow())




pct_of_total_disposal <- dt %>% filter(county_id %in% counties_that_exclude_threshold) %>% ungroup %>% summarise(t=sum(tons)) %>% pluck("t") / 
dt %>% filter(!county_id %in% counties_that_exclude_threshold) %>% ungroup %>%  summarise(t=sum(tons))%>% pluck("t")
...
counties_with_low_disposal = counties_with_low_disposal %>% pluck("county_id") %>% unique() 
counties_with_late_year_start =counties_with_late_year_start %>% ungroup   %>% pluck("county_id") %>% unique() 

counties_with_low_disposal <- counties_with_low_disposal[!counties_with_low_disposal %in% c(counties_with_incomplete_panels %>% pluck("county_id"), counties_with_late_year_start)] %>% length()

counties_with_low_avg_disposal<- counties_with_low_avg_disposal[!counties_with_low_avg_disposal %in% c(counties_with_incomplete_panels %>% pluck("county_id"), counties_with_late_year_start)] %>% length()




fileConn<-file(paste0(figure_path, "/counties_excluded_because_of_low_disposal.txt"))
writeLines(paste0(format(scales::number(counties_with_low_disposal, accuracy = 1),big.mark=",",scientific=FALSE),''), fileConn)
close(fileConn)


fileConn<-file(paste0(figure_path, "/counties_excluded_because_of_low_avg_disposal.txt"))
writeLines(paste0(format(scales::number(counties_with_low_avg_disposal, accuracy = 1),big.mark=",",scientific=FALSE),''), fileConn)
close(fileConn)

fileConn<-file(paste0(figure_path, "/number_of_excluded_counties_due_to_thershold.txt"))
writeLines(paste0(format(scales::number(counties_that_exclude_threshold %>% length(), accuracy = 1),big.mark=",",scientific=FALSE),''), fileConn)
close(fileConn)

fileConn<-file(paste0(figure_path, "/county_years_that_threshold_catches.txt"))
writeLines(paste0(format(scales::number(100*instances_that_catch_threshold, accuracy = 0.1),big.mark=",",scientific=FALSE),'%'), fileConn)
close(fileConn)

fileConn<-file(paste0(figure_path, "/pct_of_total_disposal.txt"))
writeLines(paste0(format(scales::number(100*pct_of_total_disposal, accuracy = 0.1),big.mark=",",scientific=FALSE),'%'), fileConn)
close(fileConn)
...
plac_for_histogram4 <- power_county_for_hist("RI", dt_initial, seed) #for county-level SCs we want to keep only the SCs with R-sq>0. To do so in_sample_R2_v2 needs to change. Go to line 207 and uncomment command: "#filter(r_sq > 0) %>% #from the SC that have in sample R_sq >0" 
all_treated <- c("VT", "MA", "CA", "CT", "RI", "M1", "M2")
bans <- c(2014, 2014, 2016, 2014, 2016, 2015, 2016)
plac_for_histogram6 <- power_county_for_hist("M1", dt_initial, seed)

plac_for_histogram4 %>% power_county(., "RI")

plac_for_histogram6 %>% power_county(., "M1")
...
power_county4 <- power_county(plac_for_histogram4 %>% bind_rows(), "RI")
...
#### CHECK in_sample function, need to exclude r_sq>0 filter!! 

power2 <- read.csv("final_checks/power2_impexp_spec1_asis.csv") 
...
for (seed in c(1))
{
  ...
  write.csv(
    ...
    paste0("final_checks/power_state_plac_2025_seed", seed, "spec1.csv"), row.names=FALSE
  )
  
}


power_state_plac_data <- read.csv("power_state_plac_2025_seed2.csv") #read.csv("power_state_plac_no_r_sq_filter.csv")
...
power_state6 %>% ungroup %>%  filter(att_min == max(att_min))
...
#### Rob. Check for including treated states in the donor pool

What if the treated states were allowed to be in the donor pool? 

```{r}
dt_state_initial <- pre_processing_dt_state(power2) #state-level disposal
treated_counties_id <- unique(dt_state_initial$county_id[dt_state_initial$state_id%in% all_treated]) # treated states id 
donors_state <- unique(dt_state_initial$county_id) # all donor states
all_treated <- c("VT", "MA", "CA", "CT", "RI", "All")# Never changes
bans <- c(2014, 2014, 2016, 2014, 2016, 2015) #we assume that the aggregate ban is implemented in 2015
samp=seq(3,10) # possible values of |S|
...
power_state_plac1 <- power_state_plac_include_treated("MA", dt_state_initial, 1) # SC outcomes for MA's ban
power_state_plac2 <- power_state_plac_include_treated("CA", dt_state_initial, 1) # SC outcomes for CA's ban
power_state_plac3 <- power_state_plac_include_treated("CT", dt_state_initial, 1)# SC outcomes for CT's ban
power_state_plac4 <- power_state_plac_include_treated("RI", dt_state_initial, 1)# SC outcomes for RI's ban
power_state_plac5 <- power_state_plac_include_treated("VT", dt_state_initial, 1)# SC outcomes for VT's ban
power_state_plac6 <- power_state_plac_include_treated("All", dt_state_initial, 1) #needed for the aggregate case
...
power_state1 <- power_state_fun2(power_state_plac_data %>% filter(treated_state=="MA"), "MA") 
power_state2 <- power_state_fun2(power_state_plac_data %>% filter(treated_state=="CA", county_id!="CA"), "CA")
power_state3 <- power_state_fun2(power_state_plac_data %>% filter(treated_state=="CT"), "CT")
power_state4 <- power_state_fun2(power_state_plac_data %>% filter(treated_state=="RI"), "RI")
power_state5 <- power_state_fun2(power_state_plac_data %>% filter(treated_state=="VT"), "VT")
# 
...
Because we only have 22 available ATTs in each placebo distribution and 1/22=0.045, dropping the maximum and minimum ATT for each treated state gets confidence intervals that have 91% coverage—instead of 95%. As a robustness check, we define our placebo confidence intervals using the average of the lowest and second-lowest ATT for the lower end of each interval and the average of the highest and second-highest ATT for the higher end of the interval. These slightly wider intervals preserve all of our main analysis’ findings and potentially change the analysis's optimal |S|

Below we look at the optimal |S|, as well as the confidence intervals 
...
new_power <- 
  rbind(
    power_state_fun2_alternative(power_state_plac_data %>% filter(treated_state=="MA"), "MA") %>% ungroup %>% filter(att_min == max(att_min)), 
    power_state_fun2_alternative(power_state_plac_data %>% filter(treated_state=="CA"), "CA") %>% ungroup %>% filter(att_min == max(att_min)),
    power_state_fun2_alternative(power_state_plac_data %>% filter(treated_state=="CT"), "CT") %>% ungroup %>% filter(att_min == max(att_min)), 
    power_state_fun2_alternative(power_state_plac_data %>% filter(treated_state=="RI"), "RI") %>% ungroup %>% filter(att_min == max(att_min)), 
    power_state_fun2_alternative(power_state_plac_data %>% filter(treated_state=="VT"), "VT") %>% ungroup %>% filter(att_min == max(att_min))
  )

new_xy_data <- 
  # need to load functions from xy.R
  rbind(
    xy_plot_data_function("CA",new_power %>% filter(treated_state=="CA") %>% pluck("sample_size")-2,6), 
    xy_plot_data_function("CT",new_power %>% filter(treated_state=="CT") %>% pluck("sample_size")-2,6),
    xy_plot_data_function("MA",new_power %>% filter(treated_state=="MA") %>% pluck("sample_size")-2,6),
    xy_plot_data_function("RI",new_power %>% filter(treated_state=="RI") %>% pluck("sample_size")-2,6),
    xy_plot_data_function("VT",new_power %>% filter(treated_state=="VT") %>% pluck("sample_size")-2,6)
  )
...
fileConn<-file(paste0(figure_path, "/ca_att_wide.txt"))
writeLines(paste0(format(scales::number(
  new_ATTs %>% filter(treated_state=="CA") %>% pull("actual_treatment_effect")*100
  , accuracy = 0.1),big.mark=",",scientific=FALSE),'%'), fileConn)
close(fileConn)

fileConn<-file(paste0(figure_path, "/ct_att_wide.txt"))
writeLines(paste0(format(scales::number(
  new_ATTs %>% filter(treated_state=="CT") %>% pull("actual_treatment_effect")*100
  , accuracy = 0.1),big.mark=",",scientific=FALSE),'%'), fileConn)
close(fileConn)

fileConn<-file(paste0(figure_path, "/ma_att_wide.txt"))
writeLines(paste0(format(scales::number(
  new_ATTs %>% filter(treated_state=="MA") %>% pull("actual_treatment_effect")*100
  , accuracy = 0.1),big.mark=",",scientific=FALSE),'%'), fileConn)
close(fileConn)

fileConn<-file(paste0(figure_path, "/ri_att_wide.txt"))
writeLines(paste0(format(scales::number(
  new_ATTs %>% filter(treated_state=="RI") %>% pull("actual_treatment_effect")*100
  , accuracy = 0.1),big.mark=",",scientific=FALSE),'%'), fileConn)
close(fileConn)

fileConn<-file(paste0(figure_path, "/vt_att_wide.txt"))
writeLines(paste0(format(scales::number(
  new_ATTs %>% filter(treated_state=="VT") %>% pull("actual_treatment_effect")*100
  , accuracy = 0.1),big.mark=",",scientific=FALSE),'%'), fileConn)
close(fileConn)


# all the results continue to hold, with CT moving much closer to 0, at -0.8%
```


### P values
Make sure that the filename you choose here is the same as the filename chosen in the xy plot, used to generate the ATTs
```{r}
filename = "power_state_plac_2025_seed5.csv"
power_state_plac_data <- read.csv(filename)
bt_with_power_data = read.csv("bt_with_power_data.csv")
...
p_ma <- p_values_fun (power_state_plac_data %>% filter(treated_state=="MA"), "MA") %>% pluck("p")
p_ca <- p_values_fun (power_state_plac_data %>% filter(treated_state=="CA"), "CA") %>% pluck("p")
p_ct <- p_values_fun (power_state_plac_data %>% filter(treated_state=="CT"), "CT") %>% pluck("p")
p_vt <- p_values_fun (power_state_plac_data %>% filter(treated_state=="VT"), "VT") %>% pluck("p")
p_ri <- p_values_fun (power_state_plac_data %>% filter(treated_state=="RI"), "RI") %>% pluck("p")
p_all <- p_values_fun (pool_estimates %>% bind_rows() %>% mutate(treated_state="All") %>% rename(county_id=i) %>% mutate(donor_number="donor_1"), "All") %>% pluck("p")
...
power_state_mean_placebo <- 
  ...
  group_by(sample_size, treated_state) %>% 
  summarise(att_median = mean(att))
...
writeLines(paste0(format(scales::number(mean_ma, accuracy = 0.1),big.mark=",",scientific=FALSE),'%'), fileConn)
close(fileConn)
fileConn<-file(paste0(figure_path, "/mean_ca.txt"))
writeLines(paste0(format(scales::number(mean_ca, accuracy = 0.1),big.mark=",",scientific=FALSE),'%'), fileConn)
close(fileConn)
fileConn<-file(paste0(figure_path, "/mean_ct.txt"))
writeLines(paste0(format(scales::number(mean_ct, accuracy = 0.1),big.mark=",",scientific=FALSE),'%'), fileConn)
close(fileConn)
fileConn<-file(paste0(figure_path, "/mean_ri.txt"))
writeLines(paste0(format(scales::number(mean_ri, accuracy = 0.1),big.mark=",",scientific=FALSE),'%'), fileConn)
close(fileConn)
fileConn<-file(paste0(figure_path, "/mean_vt.txt"))
writeLines(paste0(format(scales::number(mean_vt, accuracy = 0.1),big.mark=",",scientific=FALSE),'%'), fileConn)
close(fileConn)
...
writeLines(paste0(format(scales::number(mape_ma_upper, accuracy = 0.1),big.mark=",",scientific=FALSE),'%'), fileConn)
close(fileConn)
fileConn<-file(paste0(figure_path, "/mape_ma_lower.txt"))
writeLines(paste0(format(scales::number(mape_ma_lower, accuracy = 0.1),big.mark=",",scientific=FALSE),'%'), fileConn)
close(fileConn)

fileConn<-file(paste0(figure_path, "/mape_ca_upper.txt"))
writeLines(paste0(format(scales::number(mape_ca_upper, accuracy = 0.1),big.mark=",",scientific=FALSE),'%'), fileConn)
close(fileConn)
fileConn<-file(paste0(figure_path, "/mape_ca_lower.txt"))
writeLines(paste0(format(scales::number(mape_ca_lower, accuracy = 0.1),big.mark=",",scientific=FALSE),'%'), fileConn)
close(fileConn)

fileConn<-file(paste0(figure_path, "/mape_ct_upper.txt"))
writeLines(paste0(format(scales::number(mape_ct_upper, accuracy = 0.1),big.mark=",",scientific=FALSE),'%'), fileConn)
close(fileConn)
fileConn<-file(paste0(figure_path, "/mape_ct_lower.txt"))
writeLines(paste0(format(scales::number(mape_ct_lower, accuracy = 0.1),big.mark=",",scientific=FALSE),'%'), fileConn)
close(fileConn)

fileConn<-file(paste0(figure_path, "/mape_ri_upper.txt"))
writeLines(paste0(format(scales::number(mape_ri_upper, accuracy = 0.1),big.mark=",",scientific=FALSE),'%'), fileConn)
close(fileConn)
fileConn<-file(paste0(figure_path, "/mape_ri_lower.txt"))
writeLines(paste0(format(scales::number(mape_ri_lower, accuracy = 0.1),big.mark=",",scientific=FALSE),'%'), fileConn)
close(fileConn)

fileConn<-file(paste0(figure_path, "/mape_vt_upper.txt"))
writeLines(paste0(format(scales::number(mape_vt_upper, accuracy = 0.1),big.mark=",",scientific=FALSE),'%'), fileConn)
close(fileConn)
fileConn<-file(paste0(figure_path, "/mape_vt_lower.txt"))
writeLines(paste0(format(scales::number(mape_vt_lower, accuracy = 0.1),big.mark=",",scientific=FALSE),'%'), fileConn)
close(fileConn)
...
power_state_pass1 <- power_state_plac("MA", dt_state_initial, 5)
power_state_pass2 <- power_state_plac("CA", dt_state_initial, 5)
power_state_pass3 <- power_state_plac("CT", dt_state_initial, 5)
power_state_pass4 <- power_state_plac("RI", dt_state_initial, 5)
power_state_pass5 <- power_state_plac("VT", dt_state_initial, 5)
