...
write.csv(plac_for_histogram4 %>% bind_rows(), "plac_for_histogram4_seed5.csv", row.names=FALSE)
write.csv(plac_for_histogram6 %>% bind_rows(), "plac_for_histogram6_seed5.csv", row.names=FALSE)
...
```{r}
plac_for_histogram4 <- read.csv("plac_for_histogram4_seed5.csv")
plac_for_histogram6 <- read.csv("plac_for_histogram6_seed5.csv")
mae_placebo <- 
  rbind(
    plac_for_histogram4 %>% 
...
power_state1_alt <- power_state_fun2_alternative(power_state_plac_data %>% filter(treated_state=="MA"), "MA") %>% ungroup %>% filter(att_min == max(att_min)) # the optimal number of donors remains the same (9)
...
power_state3_alt <- power_state_fun2_alternative(power_state_plac_data %>% filter(treated_state=="CT"), "CT")%>% ungroup %>% filter(att_min == max(att_min)) # the optimal number of donors changes from 3 to 5 (ATT is: -5.43)
power_state4_alt <- power_state_fun2_alternative(power_state_plac_data %>% filter(treated_state=="RI"), "RI")%>% ungroup %>% filter(att_min == max(att_min)) # the optimal number of donors changes from 3 to 8 (ATT is: -4.31)
power_state5_alt <- power_state_fun2_alternative(power_state_plac_data %>% filter(treated_state=="VT"), "VT")%>% ungroup %>% filter(att_min == max(att_min)) # the optimal number of donors remains the same (5)
