...
check_fun <- function (state_id_given)
{
  imports %>% 
...
validate_imports <- lapply(data_states, check_fun) %>% bind_rows() %>% as_tibble()

validate_imports %>% 
  # filter(!is.na(from_exports), 
  #        from_imports > 10000, 
  #        from_exports > 10000) %>% 
  left_join(
    power2 %>% 
      filter(year>2010, type%in%c("disposal", "msw_disposed")) %>% 
      group_by(state_id, year) %>% 
      summarise(tons= sum(tons, na.rm = TRUE)), 
  ...
  mutate(
    val=(from_imports-from_exports)/ tons) %>% 
  group_by(state_id_given) %>% 
  summarise(m=median(val,na.rm=TRUE)) %>% summarise(mean(m))
