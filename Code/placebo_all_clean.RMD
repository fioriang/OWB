...
in_sample_R2_v2_counties <- function (k, dt, donors,iterations_scale, option, ban_year, offset, samp, seed)
{
  set.seed(seed)
  ###
  # This function creates the SC using the identification method we describe in the paper 
  ...
  for (f in seq_along(samp)) {
    sample_size <- samp[f]
    iterations  <- iterations_scale * sample_size #100000
...
power_county_for_hist <- function(treated_state, dt_initial, seed)
{
  ...
  plac <- future_lapply(
    seq_along(donors),
    in_sample_R2_v2_counties,
    dt,
    donors = donors,
    iterations_scale = 10000,
    option = "V2",
    ban_year = ban_year,
    offset = 3,
    samp = samp,
...
power_state_plac <- function(treated_state, dt_state_initial, seed)
{
  ...
  plac <- future_lapply(
    ...
    samp = samp,
    seed = seed,
    future.seed = TRUE #FALSE to exactly re-produce the previous script
  )  
...
plac_for_histogram4 <- power_county_for_hist("RI", dt_initial, seed) #for county-level SCs we want to keep only the SCs with R-sq>0. To do so in_sample_R2_v2 needs to change. Uncomment command: "#filter(r_sq > 0) %>% #from the SC that have in sample R_sq >0" 
...
for (pl_seed in 1:10){
  power2 <- read.csv("final_checks/power2_impexp_spec2_ut_nmsw.csv") 
  ...
  start <- Sys.time()
  power_state_plac1 <- power_state_plac("MA", dt_state_initial, pl_seed) # SC outcomes for MA's ban
  power_state_plac2 <- power_state_plac("CT", dt_state_initial, pl_seed) # SC outcomes for MA's ban
  power_state_plac3 <- power_state_plac("CA", dt_state_initial, pl_seed) # SC outcomes for MA's ban
  power_state_plac4 <- power_state_plac("RI", dt_state_initial, pl_seed) # SC outcomes for MA's ban
  power_state_plac5 <- power_state_plac("VT", dt_state_initial, pl_seed) # SC outcomes for MA's ban
  power_state_plac6 <- power_state_plac("All", dt_state_initial,pl_seed) # SC outcomes for MA's ban
  end <- Sys.time()
  end - start
  ...
  write.csv(
    rbind(
      ...
      power_state_plac2%>% bind_rows() %>% mutate(treated_state="CT"), 
      power_state_plac3%>% bind_rows() %>% mutate(treated_state="CA"), 
    ...
    paste0("final_checks/power_state_plac_spec2_ut_nmsw", pl_seed, ".csv"), row.names=FALSE
  )
}
...
```{r}
filename = "final_checks/power_state_plac_spec2_ut_nmsw.csv"
...
new_xy_data <- 
  ...
  rbind(
    ...
    xy_plot_data_function("CT",new_power %>% filter(treated_state=="CT") %>% pluck("sample_size")-2,1),
    xy_plot_data_function("MA",new_power %>% filter(treated_state=="MA") %>% pluck("sample_size")-2,1),
    xy_plot_data_function("RI",new_power %>% filter(treated_state=="RI") %>% pluck("sample_size")-2,1),
