...
ghg_data_path <- "/Users/fian4421/Library/CloudStorage/Dropbox/Organic Waste Bans/06. Post SYP/06. Emissions/GHG_Data"
...
gas_fc <- 
  ...
  #group_by(facility_id) %>% filter(n()==9) %>% ungroup %>% 
...
power_gas <- function (i, treated_state_2, f, seed){
  xy_plot_data_function(donors[i], f, seed, treated_state_2) %>% as_tibble() %>% 
    ...
    summarise(att2=mean(att2), mape = mean(mape), r_sq = mean(r_sq)) %>%
...
{
  power_gas_res_ma <- lapply(1:n, power_gas, "MA",f=f_chosen, seed=1)
  power_gas_res_ct <- lapply(1:n, power_gas, "CT",f=f_chosen, seed=1)
  power_gas_res_ca <- lapply(1:n, power_gas, "CA",f=f_chosen, seed=1)
  power_gas_res_ri <- lapply(1:n, power_gas, "RI",f=f_chosen, seed=1)
  power_gas_res_vt <- lapply(1:n, power_gas, "VT",f=f_chosen, seed=1)
...
power_gas_dist_fun <- function(power_gas_dist_fun_res)
{
  power_gas_dist_fun_res %>% bind_rows %>% 
    ...
    summarise(
      min = sort(att)[2], 
      max = sort(att, decreasing = TRUE)[2]
      ) %>% 
    group_by(
      treated_state_2
    ) %>% 
    #filter(treated_state_2 =="CA" & sample_size>3 | treated_state_2!="CA") %>% 
    # for CA choose choose sample size =4 instead of size=3 because the min att_placebo is almost the same while the max att_placebo is 0.3 for sample size = 4 and 0.433 for sample size =3 
    filter(min==max(min))
}

power_gas_dist_fun_res <- lapply(1:9, power_gas_dist_fun_plac) #1:9 are the different sample sizes 

power_gas_dist_fun_res %>% power_gas_dist_fun #decide |S|

write.csv(power_gas_dist_fun_res %>% bind_rows(), "power_gas_res_spec1.csv", row.names=FALSE)
power_gas_dist_fun_res <- read.csv("power_gas_res_spec1.csv")


chosen_sample_size_CA = power_gas_dist_fun_res%>% power_gas_dist_fun %>% filter(treated_state_2=="CA") %>% pluck("sample_size") %>% -1
chosen_sample_size_CT = power_gas_dist_fun_res%>% power_gas_dist_fun %>% filter(treated_state_2=="CT") %>% pluck("sample_size") %>% -1
chosen_sample_size_MA = power_gas_dist_fun_res%>% power_gas_dist_fun %>% filter(treated_state_2=="MA") %>% pluck("sample_size") %>% -1
chosen_sample_size_RI = power_gas_dist_fun_res%>% power_gas_dist_fun %>% filter(treated_state_2=="RI") %>% pluck("sample_size") %>% -1
chosen_sample_size_VT = power_gas_dist_fun_res%>% power_gas_dist_fun %>% filter(treated_state_2=="VT") %>% pluck("sample_size") %>% -1

sc_data_ma <-xy_plot_data_function("MA", chosen_sample_size_MA, 2, "MA") #f is sample size -1
sc_data_ct <-xy_plot_data_function("CT", chosen_sample_size_CT, 2, "CT")
sc_data_ca <-xy_plot_data_function("CA", chosen_sample_size_CA, 2, "CA") 
sc_data_ri <-xy_plot_data_function("RI", chosen_sample_size_RI, 2, "RI")
sc_data_vt <-xy_plot_data_function("VT", chosen_sample_size_VT, 2, "VT")
...
write.csv(sc_data, "sc_data_ghg_spec1.csv", row.names = FALSE)
sc_data <- read.csv("sc_data_ghg_spec1.csv")

actual_effects <- 
  sc_data %>% 
  group_by(county_id) %>% 
  filter(mape==min(mape)) %>% 
  summarise(actual=mean(att2)) %>% 
...
fileConn<-file(paste0(figure_path, "/ma_effect_ghg_spec1.txt"))
...
power_gas_res_plot <- 
  ...
  left_join(expected_effects, by = c("state_id"="county_id")) %>% ungroup %>% 
  mutate(
    state_id = fct_recode(
      state_id,
      "Vermont" = "VT",
  ...
  mutate(
    state_id = factor(state_id, levels = c("Vermont", "Rhode Island", "Massachusetts", "Connecticut", "California"))
  ) %>% 
  ggplot()+
  aes(y= state_id, x= 100*actual, group = 1)+
  geom_errorbar(aes(xmin = 100*min, xmax = 100*max, color = "Placebo"), 
                width = 0.2, size = 0.5) +
  geom_point(aes(color = "Estimate"), size = 1.5) +
  geom_errorbar(aes(xmin = -100*expected_effect, xmax =-100*expected_effect, color = "Our Exp."), 
...
############## S1.  Save Figure (spec 1) #################
ggsave(
  xy_and_power_gas, filename = "xy_and_power_gas_spec1.pdf", device = cairo_pdf,
  path= figure_path,
  width = 8, height = 7, units = "in")


############### S2. Preparing the alg (spec 2) #############################
...
{  
  ...
  set.seed(seed)
  if (treated_state %in% all_treated){ban_year <- bans[which(all_treated == treated_state)]}else{ban_year <- bans[which(all_treated == treated_state_2)]}
  # in spec 2, we shift the ban year to the closest calendar year, for MA, VT (implementation in Oct and July of 2014) we shift the ban year to 2015
  if (treated_state_2 %in% c("VT", "MA")){ban_year = ban_year +1}
...
write.csv(power_gas_dist_fun_res %>% bind_rows(), "power_gas_res_spec2.csv", row.names=FALSE)
power_gas_dist_fun_res <- read.csv("power_gas_res_spec2.csv")
...
write.csv(sc_data, "sc_data_ghg_spec2.csv", row.names = FALSE)
sc_data <- read.csv("sc_data_ghg_spec2.csv")
...
fileConn<-file(paste0(figure_path, "/ma_effect_ghg_spec2.txt"))
writeLines(paste0(format(round(ma_effect_ghg,1),big.mark=",",scientific=FALSE),'%'), fileConn)
close(fileConn)
...
############## S2. Save Figure (spec 2) #################
ggsave(
  xy_and_power_gas, filename = "xy_and_power_gas_spec2.pdf", device = cairo_pdf,
  path= figure_path,
  width = 8, height = 7, units = "in")



############### S3. Preparing the alg (spec 3) #############################
...
{  
  ...
  set.seed(seed)
  if (treated_state %in% all_treated){ban_year <- bans[which(all_treated == treated_state)]}else{ban_year <- bans[which(all_treated == treated_state_2)]}
  # in spec 2, we shift the ban year to the calendar year, for MA, VT, CA (implementation in Oct '14, July '14, Apr '16) we shift the ban year to 2014 for MA and VT and to 2016 for CA
...
write.csv(power_gas_dist_fun_res %>% bind_rows(), "power_gas_res_spec3.csv", row.names=FALSE)
power_gas_dist_fun_res <- read.csv("power_gas_res_spec3.csv")
...
write.csv(sc_data, "sc_data_ghg_spec3.csv", row.names = FALSE)
sc_data <- read.csv("sc_data_ghg_spec3.csv")
...
fileConn<-file(paste0(figure_path, "/ma_effect_ghg_spec3.txt"))
writeLines(paste0(format(round(ma_effect_ghg,1),big.mark=",",scientific=FALSE),'%'), fileConn)
close(fileConn)
...
############## S2. Save Figure (spec 2) #################
ggsave(
  xy_and_power_gas, filename = "xy_and_power_gas_spec3.pdf", device = cairo_pdf,
  path= figure_path,
  width = 8, height = 7, units = "in")
