...
png(paste0(figure_path, "/map_plot_county_SC.png"), bg = "transparent", width = 15, height = 15, units = "in", res=1200)
print(map_plot_county)
dev.off()
```
...
dt_anova <- 
  dt_anova %>% group_by(state_id, year) %>% summarise(tons_pc = sum(tons)/sum(pop))
...
library(fixest)
dt_state_initial <- pre_processing_dt_state(power2)
fixest::feols(tons_pc ~ 0|state_id, data=dt_anova) %>% summary()
fixest::feols(tons_pc ~ 0|state_id, data=dt_anova) %>% fixef(se=TRUE)
# R sq. from here!!!!


feols(tons_pc ~ 0 | state_id, data = dt_anova) %>% fixef()
lm(tons_pc ~ 0 + as.factor(state_id), dt_anova) %>% summary()
# Estimates of fixed effects from here!



feols(tons_pc ~ 0+ as.factor(state_id), data = dt_anova %>% filter(!state_id%in% c("NV", "NY")))%>% etable(vcov="hetero", tex=FALSE)


dt_anova %>% group_by(state_id) %>% summarise(m=mean(tons_pc, na.rm=TRUE)) %>% ungroup %>% filter(!state_id%in% c("NV", "NY")) %>%  #because not full coverage
  mutate(mean_m=mean(m, na.rm=TRUE)) %>% 
  filter(m==max(m, na.rm=TRUE)|m==min(m, na.rm=TRUE)) %>% 
  mutate(
    (m-mean_m)/mean_m
  )


dt_anova %>% group_by(year) %>% summarise(m=mean(tons_pc, na.rm=TRUE))
