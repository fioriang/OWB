...
I deleted sections "fixing CT" and "CT vol2" on June 4th
...
data_availability <- 
  ...
  geom_point(aes(x=year, y=passage_cross),color = "plum")+
  facet_wrap(
    vars(state_id), 
    ncol= 7
  ) + 
...
data_availability <- function (chosen_region)
{
  ...
  dt_in %>% mutate(
    ...
    scale_color_manual(values=c("goldenrod", "#417c5b"), na.translate = FALSE)+
    #scale_y_continuous (breaks = c(2005, 2015))+
    theme(
      panel.border = element_blank(),
...
map_plot_county <- 
  ...
  geom_line(
    data = tibble(x=seq(-124, -122), y=52, year = 2008), aes(x=x, y=y), color = "#417c5b", linewidth=2)+
  geom_text(
    data = tibble(x=-118, y=52, year = 2008, lab = "Treated"), aes(x=x, y=y, label=lab), family = "Helvetica", color = ut_colors[4], size=5.5)+
  theme(
...
dt_anova %>% 
  ...
  left_join(
    tibble(
      #chat gpt data
      total_pop = c(283369548, 286944137, 290326418, 293670383, 296941146, 300239335, 303504863, 306771529, 309988540, 313181047, 316329820, 319439055, 322401407), 
      year = 2006:2018
    ), 
    by = "year"
  ) %>% 
